...
> **Status: no longer maintained.** The last release is [v1.0.106](https://github.com/xcinnamoroll/summon-heroes-trainer/releases/tag/v1.0.106). The trainer still works as of that version, but no further updates, bug fixes, or compatibility patches are planned. If the in-game UI changes, image-detection-based features (Story Mode, Auto Chest, Custom Pathing triggers, PvP Shop auto-buy) may need re-captured reference images to keep working. Forks welcome.
